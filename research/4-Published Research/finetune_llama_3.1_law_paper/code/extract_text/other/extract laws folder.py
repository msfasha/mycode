...
# Function to extract Arabic text from the <article> tag and directly write it to a file
def extract_and_save_arabic_text_from_html(file_path, output_file_path):
    with open(file_path, 'r', encoding='utf-8') as file:
        ...
        # Loop through each <article> and extract Arabic text
        for article in articles:
            for tag in article.find_all(string=True):
                if contains_arabic(tag):
                    arabic_text.append(tag.strip())
    
    # If we found Arabic text inside the <article>, write it to a separate file
    if arabic_text:
...
# Function to iterate through all folders and find HTML files with Arabic content in <article> tags
