...
for tag in discardElements:
    pattern = re.compile(r'<\s*%s\b[^>]*>.*?<\s*/\s*%s>' %
...
# sys.exit(1)
    ...
    if 'bzip2' in ftypes:
        ...
        process_data('bzip2', f, output_sentences,
                     vital_titles, incubator, vital_tags)
        output_sentences.close()

    elif 'gzip' in ftypes:
        print('File detected as being a gzip.')
        f = gzip.GzipFile(fname, mode='r')
        process_data('gzip', f, output_sentences,
                     vital_titles, incubator, vital_tags)
        output_sentences.close()
    else:
        with open(args.infn) as infile:
            process_data('xml', infile, output_sentences,
                         vital_titles, incubator, vital_tags)
        output_sentences.close()
